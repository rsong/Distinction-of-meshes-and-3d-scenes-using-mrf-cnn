function feats = cnn_shape_view( imList, model, layers, varargin )
%CNN_SHAPE_VIEW Compute and save CNN activation features
%
%   imList:: 
...
if ~isempty(viewpoolIdx), 
    ...
    nViews = length(net.meta.classes.name);
end
...
for fi = 1:numel(layers.name),
    featCell{fi} = zeros(layers.sizes(1,fi),layers.sizes(2,fi),...
        layers.sizes(3,fi),nSubWins*batchSize* nViews, 'single'); %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% maybe need to be modified
end
